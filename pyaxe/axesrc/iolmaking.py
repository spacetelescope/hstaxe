...
class ProjectionList:
    def __init__(self, header_value='', dim_info=''):
        ...
        header_value : str
            ...
            drizzled image, this is used with the D*DATA
            keywords saved to the header by astrodrizzle
        dim_info : str
    ...
    def make_grismcat(self, drizzle_image="", catalog=None):
        ...
        if not isinstance(catalog, Table):
            raise aXeError("Expected input table to be an astropy table")

        print("\n >>>> Working on Input Object List: {0:s} >>>>\n"
              .format(self.iol_name))

        # now translate ra dec to new image pixel points
        print("Extracting wcs from {0}".format(self.header_value))
        ...
        for row in range(len(catalog)-1, -1, -1):
            x, y, trad = catalog['X_IMAGE'][row], catalog['Y_IMAGE'][row], catalog['THETA_IMAGE'][row]

            # compute shift vector
            if translate:
                trad = math.radians(trad)
            # 10.0 is a made up scaling length to use to
            # get the angle precision
            shifted_x = x + 10.0 * math.cos(trad)
            shifted_y = y + 10.0 * math.sin(trad)
            ...
            if ((x_ori >= self.dim_info[0] and x_ori <= self.dim_info[1]) and \
               (y_ori >= self.dim_info[2] and y_ori <= self.dim_info[3])):
                ...
                dx = shifted_x - x_ori
                dy = shifted_y - y_ori
                angle = math.atan2(dy, dx)  # compute local angle
            ...
            print("{0} master_x={1}  master_y={2}  theta={3} dx={4} dy={5} angle={6}".format(row, x, y, trad, dx, dy, angle))
